...
The long version. This was the README until it reached 525 lines, at which point it stopped being a
front door and became a log — which is what it always was. Nothing here is cut from the project; it
is the reasoning, the corrections and the numbers behind the claims on the
[front page](../README.md).

It is kept deliberately, because a quarter of it is a record of things this project got **wrong** and
then fixed: a theorem that was briefly false, a fabricated headline output, a bug-finder that
explored nothing, a cost axis that mixed two conventions, an airlock that was inert under a wall
clock. A reader who wants to know whether the numbers on the front page can be trusted should read
